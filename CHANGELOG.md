...
## v0.1.9
- 更新检查更可靠、更直观：左上角版本号现在会定期自动复查（面板开着时也能发现新版，不必手动刷新），网络抖动导致的失败会自动重试，不再一次失败就整段时间不提示。
- 已是最新版本时，鼠标悬停版本号会显示「已是最新版本」，明确告知检查已生效、当前已最新（此前没有任何反馈，容易被误以为功能失灵）。有新版时仍为柠檬黄高亮 + 悬停「检测到新版本：vX.X.X」+ 更新按钮。
